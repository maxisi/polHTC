...
subname = 'subs/generic_gauss.sub'

dagname = 'dags/fullgauss_%(det)s%(run)s_%(psrIN)s.dag' % locals()
...
analysis_folder = '/analyses/gauss' # this serves to separate unrelated executables

# get hostname to determine what server we are on
cluster = g.Cluster()

# define scratch space
scratch_dir = cluster.scratch_dir + 'fullgauss_$(psr)_$(det)$(run)_$(injkind)$(pdif)s'
